...
// Abstract base class matching the spec interface from the lecture.
...
public:
    ...
    // Scheduler convenience — each process may have a different memory size
    virtual bool        allocateForProcess(const std::string& processName, size_t memSize) = 0;
    ...
    // Memory access for READ/WRITE instructions
    // Returns true on success, false on access violation
    virtual bool readMemory(const std::string& processName, uint32_t address, uint16_t& outValue) = 0;
    virtual bool writeMemory(const std::string& processName, uint32_t address, uint16_t value) = 0;
